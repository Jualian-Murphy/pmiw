...
class Jugador {
  ...
  mover(tecla) {
    ...
    let paso = 40;
    if (tecla === 'ArrowUp') this.y -= paso;
    if (tecla === 'ArrowDown') this.y += paso;
    if (tecla === 'ArrowLeft') this.x -= paso;
    if (tecla === 'ArrowRight') this.x += paso;
...
class Corriente {
  ...
  crearBurbujas(cantidad, velocidad) {
    let tamano = 50;
    let distanciaSeguraEntreBurbujas = 80;
    ...
    for (let i = 0; i < cantidad; i++) {
      ...
      do {
        ...
        this.burbujas.some(b => abs(b.x - x) < distanciaSeguraEntreBurbujas) ||
        (abs(x - jugadorX) < 100 && this.y > height - 100 && intentos < maxIntentos)
      );
...
class Juego {
  ...
  mostrarInicio() {
    ...
    text("Hecho por: Julian Murphy y Catalina Schwerdt", width / 2, height - 30);
  }
